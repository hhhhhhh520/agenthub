...
### ISSUE-ORC-003: 监督机制缺失 — ⚪不实施 (2026-05-31)

**位置**: `src/app/api/sessions/[id]/chat/route.ts:577-623`

设计要求："正常运行时沉默，只在关键节点介入"

**评估结论**: 不作为独立功能实施，现有机制已足够。
- 纯规则检测（关键词匹配）：Agent 正常执行中遇到错误并自行处理时会误报，误报率高
- LLM 流式监控：成本过高（3 分钟任务额外 6 次 LLM 调用），且用户不接受
...
| 监督者：沉默观察 | ⚪ 不实施 | 评估后认为现有事后审查+纠偏重试已足够，纯规则检测误报率高，LLM 监控成本过高 |
