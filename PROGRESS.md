# AgentHub 项目进度
> 创建时间: 2026-05-22 | 最后更新: 2026-08-07
...
| P0 重放 bug 修复(已完成任务不重跑) | 根因:handleExecution success 循环遍历 executeTaskBatch 返回全部 results(含 priorResults 预装的历史已完成任务),redo/续跑/多批执行时旧任务被重跑 monitoring LLM + 重复写 DB + 覆盖 trace/correctionCount,纠偏置 pending 后 correctionCount 每批重置使 max-2 上限失效 → 无界重执行。主修法:executeTaskBatch 返回新增 preloadedIds(仅标记"priorResults 中本批未执行"的历史任务),handleExecution success 循环跳过。pre-commit 三视角审查:生命周期抓 ❌ 纠偏重试 overlap 被误跳过(新结果丢弃、任务滞留 pending)→ batchTaskIds 排除本批任务修复+守卫。3 真回归守卫(历史跳过/preloadedIds 形状/overlap 不误标),红绿验证通过。939→942 测试全绿,src零tsc。已知残留:纠偏置 pending 时 allResults 旧 result 未清、重跑失败被陈旧 result 覆盖(既有行为) | 2026-08-07 |
