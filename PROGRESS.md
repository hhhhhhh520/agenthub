...
| 🔴高 | P7-A mock 判定误判修复 | run.test.ts mock `sp.includes('架构师')` 误判 handleOrchestratorChat（其 systemPrompt 含"当前会话中的 Agent：- 架构师"字样）→ 8 条 off-C defect 伪影。修法：架构师分支改 `agent?.name==='架构师'` 精确判定；修后重跑 A 全格 + off-C 格下方向性结论 | 待做(2026-08-22) |
