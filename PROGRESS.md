...
| P9-乙 T5 强模型全矩阵 45-run（2026-08-30） | 三臂（off/on/on-seqgate+verify）×A/B/C×5，deepseek@xfyun。**首发射暴露计划接线缺口**：T3 为保 P6 2×2 语义把 configs 留成 5 配置、T5 未指定三臂选择器→"无参全矩阵"实际跑 75 格；跑到 21 行时人工巡检发现（off/on 之间冒出 no-verify 行），中止弃用该批（metrics.p9b-aborted-21.bak.jsonl），补 `P9_ARMS=1` 门控（config.ts isP9ArmsOnly 严格相等+run.test.ts 循环过滤+launcher 无参分支+2 条针对性测试，455591a），双向收集数验证（未设=75 格/=1=45 格）后重跑。**结果 45/45、三臂+全批 detectBatchContamination 判阴、DB trace 中毒扫描 0**：A：off 3/5、on 2/5、**seqgate 4/5**；B：5/5×3（天花板）；C：off 0/5、on 0/5、**seqgate 4/5**。McNemar seqgate 增量：**C b=0 c=4 p≈0.046 显著**（A方向 2→4 p=0.317 不显著；B 无空间）；gate 触发 9/15 runs（A×5+C×4，C-s0 未触发即 no-pass）；seqgate 臂非法提案率 1.13/run vs ON 0.20/OFF 0.13（闸门踢回诱发重提议=H1 行为层证据）。状态机主效应 OFF vs ON 依旧无差（延续 P8）。**裁决：H1✅、H2✅（传导仅在 C 显著）、H3 原句"不影响 C"被本批否定、v3.1 修正版（C 同族可移动）证实——seqgate 的真实主战场是 C 捷径任务**。defect 3/45（on-A no-pass、seqgate-A error、seqgate-C no-pass）均真实行为非中毒。数据 metrics.p9b-strong-20260829.bak.jsonl、报告 report-p9b-strong.md、日志 p9b-strong.log。附带记录：末 7 个 work 目录 afterAll teardown 无 warn 但残留（疑 CLI 迟到写重建，P10 观察项）；中断批 24 目录+存量 398 待一次性清理 | 2026-08-30 |
...
| 任务 | 状态 |
|------|------|
| P8 收尾推送确认 | 33 个本地 commit 未推（P7/P8/P9-丙/P9-乙 全部+桥/pilot 工具）；两把实验 key 轮换已被用户 2026-08-30 拍板豁免（"不用换,直接用"） |
| P9-丙 跨批分析（2026-08-24 完成） | `analyze-cross-batch.ts`+22 测试（SDD 六 task 全过评审+终审）；产出 `results/report-cross-batch.md`：三批统一指纹表、缺边类型学 4 签名、corrections 成分分解（**gate 占 corr≈98.6%**——状态机捆绑件的实际工作成分是确定性补缺失而非纠非法，P9 作用面错位假说的直接证据）；spec `docs/superpowers/specs/2026-08-23-p9-surface-matrix-design.md`（v3.1） |
| P9-乙 seqgate 矩阵（详见已完成表 2026-08-28/08-30 行） | 代码+冒烟+**T5 强模型 45-run 已完成**（H3 修正版证实：seqgate 显著移动 C p≈0.046）。**下一步**：T6 弱模型 pilot（F1 前置=轮换讯飞/OpenRouter 两把 key → 讯飞 GET /v1/models 列候选 → 探带格 off+verify×A×5 中间带 pass 20-80% → 过带铺三臂 45-run）→ 收官文档同步+推送确认 |
