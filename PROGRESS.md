...
| P6(=全矩阵) 2×2 析因实验 | P5 残留 6 项修复（**trace 不对称 A0 最先**+A1/A2 罐头差异化+A3 两套超时放宽+A4 upsert+模型钉死 deepseek-v4-flash+A5 seed 排序）+ verify 开关（`EXPERIMENT_VERIFY=off` 只包 alignment.ts verify 自动创建块，生产默认未设零影响）+ 2×2 全矩阵 harness（状态机 on/off × verify 有/无 = 4 配置 × 3 任务 × 5 seed = **60 run**，12 批可续跑，report 含状态机/verify 主效应+交互）。生产改动 2 处（chat-router A0 抑制修正+alignment verify 开关），A0 回归 P4「不双记」不变量。**60-run 待真实 key 执行**。spec: docs/superpowers/specs/2026-08-15-p6-full-matrix-design.md | 2026-08-16 |
