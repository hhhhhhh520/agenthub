...
|---|---|---|---|
| **质量门禁** | CI：clippy `-D warnings`、快照测试、架构守卫、453 前端测试 | 无 `.github/`；1098 单测（2026-09-20 修复后 1130）+ E2E 仅 13 个冒烟用例；`npx eslint .` 当前 **362 errors / 83 warnings** | 有测试无门禁 |
...
| **安全底线** | 两条凭据通道（HTTP Bearer + WS protocol）+ 空 token fail-closed | 31 个路由零认证（**已拍板维持缓期：只在本机运行**，§8#1）；spawn 注入面已守卫（2026-09-20，§2.2 第 1 项）；`list_files`/`attachments` 路径校验已收口（§2.2 第 2 项）；shadow-git execSync 注入待修（🔴 §2.2 第 4 项） | spawn 已收敛；shadow-git 待修 |
...
2. ✅ **`list_files` 路径校验收口**（2026-09-20 完成）：新增 `isListDirSafe`（path-safety.ts）消除前缀同族绕过（`../project-evil`）；进一步收口 junction 泄漏——新增 `src/lib/list-dir.ts`（`listDirTree` 手动递归不跟进符号链接，`listProjectFiles` 承载工具主体）；同型缺口 `attachments/[id]` 路由裸 `startsWith` 一并换 `isPathSafe`；新增测试：累计 1098→1130（校验守卫 +5 / junction 收口 +11 / spawn 守卫 +16），全量通过。
...
4. 🔴 **shadow-git execSync 模板串注入（2026-09-20 审查发现，实测可复现，待修）**：`projectDir` 经 `POST /api/sessions` 零校验入库 → `execution.ts` → `shadow-git.ts` 的 `execSync(\`git ... "${shadowDir}" "${workDir}"\`)` 模板串拼 shell——`projectDir` 含 `"` 即可执行任意命令（审查 Agent 以标记文件实测成功）。修法：全部改 `execFileSync` 参数数组（无 shell 即无注入），紧随本批处理。
...
| 单测 1130 passed / 3 skipped（2026-09-20 复跑；v4 记录为 1098） | `npx vitest run` | 实跑 |
